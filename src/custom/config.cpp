...
static void create_key_map(Key_Map& key_map) {
    ...
    BIND(key_map, "A-F3", version_control::command_git_diff_master_this_file);
    BIND(key_map, "F4", version_control::command_git_diff_head);
    BIND(key_map, "A-F4", version_control::command_git_diff_head_this_file);
    ...
    case Buffer::TEMPORARY: {
        ...
        if (buffer->name.starts_with("*man ")) {
            ...
            return;
                   ...
                   buffer->name.starts_with("*git dm ") || buffer->name == "*git dh*" ||
                   buffer->name.starts_with("*git dh ") ||
