...
void command_shift_line_forward(Editor* editor, Command_Source source) {
    ...
    for (size_t c = 0; c < cursors.len; ++c) {
        ...
        cursor_positions.push(insertion_point.position - (end.position - start.position) + column);
        ...
        SSOStr value = buffer->contents.slice(transaction.value_allocator(), start, end.position);
